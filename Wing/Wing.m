...
station.Chord_len = var.GeoRootChord - (var.GeoRootChord-var.TipChord) * (var.FuseRad+station.SpanMesh) / var.SemiSpan; %get station chords
for i = 1:length(station.Chord_len) %scale airfoil coordinates for each station and put it to global airplane xyz coord
    % where y is direction of span, z is direction of gravity
    station.x_airfoil(i,:) = station.Chord_len(i) .* x_airfoil;
    station.z_airfoil(i,:) = station.Chord_len(i) .* y_airfoil;
...
station.SF_tot = station.SF_aero + station.SF_inertial;
station.BM_tot = station.BM_aero + station.BM_inertial;

% get aerodynamic pitching moment for each station
T_M0 = 0.5 * var.rho * var.Vmo^2 * station.Chord_len.^2 * var.Cm0; %formula from notes p54

% get the moment arms for each stations
station.a = (var.FALocation - 0.25).* station.Chord_len; %lift moment arm 
station.b = (var.CGLocation - var.FALocation).* station.Chord_len; %weight moment arm (CG - flexural axis)
...
plot([station.SpanMesh(station_idx_Engine),station.SpanMesh(station_idx_Engine)],[0,-var.EngineMass*9.81],'r',"LineWidth",2)
ylim([-200 1000]) 
...
grid on
ylabel('SF (N)')
plot(station.SpanMesh,station.SF_tot,'b')
% plot BM
subplot(3,1,2)
hold on
grid on
ylabel('BM (Nm)')
plot(station.SpanMesh,station.BM_tot,'b')
% plot T
subplot(3,1,3)
hold on
grid on
